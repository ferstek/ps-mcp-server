...
export class PrestaShopClient {
  ...
  _qs(params) {
    // PS WebServices needs literal brackets in keys (e.g. filter[field]=value)
    // URLSearchParams encodes them as %5B%5D which PS rejects
    const parts = ['output_format=JSON'];
  ...
  async getOrdersByDate(dateFrom, dateTo, limit = 100) {
    // If dateTo has no time component, extend to end of day so same-day queries work
    const dateToFull = /^\d{4}-\d{2}-\d{2}$/.test(dateTo) ? `${dateTo} 23:59:59` : dateTo;
    const qs = this._qs({
      'filter[date_add]': `[${dateFrom},${dateToFull}]`,
      date: '1',
  ...
  async getOrdersByState(stateId, limit = 100) {
    // PS WebServices doesn't support filtering by current_state directly.
    // Fetch a large batch of recent orders and filter client-side.
    const fetchLimit = Math.min(limit * 8, 500);
    const qs = this._qs({
      display: '[id,reference,total_paid_tax_incl,date_add,current_state,id_customer]',
    ...
    const filtered = (data?.orders ?? [])
      .filter((o) => String(o.current_state) === String(stateId))
      .slice(0, limit);
    return this._enrichOrderList(filtered);
  }
  ...
  async searchOrders({ dateFrom, dateTo, stateId, customerId, limit = 50 }) {
    ...
    if (dateFrom && dateTo) {
      const dateToFull = /^\d{4}-\d{2}-\d{2}$/.test(dateTo) ? `${dateTo} 23:59:59` : dateTo;
      params['filter[date_add]'] = `[${dateFrom},${dateToFull}]`;
      params.date = '1';
      params.sort = '[date_add_DESC]'; // sort only works alongside date=1 in PS
    }
    // Note: current_state is not filterable via PS WebServices — filtered client-side below
    if (customerId) params['filter[id_customer]'] = customerId;

    const data = await this._get(`/api/orders${this._qs(params)}`);
    let orders = data?.orders ?? [];
    if (stateId) orders = orders.filter((o) => String(o.current_state) === String(stateId));
    return this._enrichOrderList(orders);
  }
